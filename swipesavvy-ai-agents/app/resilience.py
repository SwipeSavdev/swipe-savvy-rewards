    ...
    async def call(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        if self.state == CircuitState.HALF_OPEN:
            if self.half_open_count >= self.config.max_half_open_calls:
                raise Exception(
                    f"Circuit breaker HALF_OPEN, max calls exceeded for {self.config.name}"
                )
...
class RetryPolicy:
    ...
    async def execute(
        self, func: Callable[..., T], *args, retriable_exceptions: tuple = (Exception,), **kwargs
    ) -> T:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.warning(
                        f"Attempt {attempt + 1} failed: {str(e)}. " f"Retrying in {delay:.2f}s..."
                    )
...
class ResilientClient:
        ...
        **kwargs,
        ...
        async def bulkhead_wrapped():
            return await self.bulkhead.execute(
                self._circuit_breaker_call, func, retriable, op_name, *args, **kwargs
            )

        try:
            result = await with_timeout(bulkhead_wrapped(), self.timeout_seconds, op_name)
            return result
        except Exception as e:
    ...
    async def _circuit_breaker_call(
        self, func: Callable[..., T], retriable: bool, op_name: str, *args, **kwargs
    ) -> T:
    ...
    return {
        "database": ResilientClient(
            ...
            circuit_breaker_config=CircuitBreakerConfig(
                failure_threshold=5, recovery_timeout=60, max_half_open_calls=3, name="database"
            ),
            retry_config=RetryConfig(max_retries=3, initial_delay=0.5, max_delay=10.0),
            timeout_seconds=30,
            bulkhead_config=BulkheadConfig(
                max_concurrent_calls=50, max_waiting_calls=100, name="database"
            ),
        ),
        "external_api": ResilientClient(
            "external_api",
            circuit_breaker_config=CircuitBreakerConfig(
                failure_threshold=3, recovery_timeout=30, max_half_open_calls=1, name="external_api"
            ),
            retry_config=RetryConfig(max_retries=2, initial_delay=1.0, max_delay=30.0),
            timeout_seconds=15,
            bulkhead_config=BulkheadConfig(
                max_concurrent_calls=20, max_waiting_calls=50, name="external_api"
            ),
        ),
        "rate_limiter": ResilientClient(
            ...
            retry_config=RetryConfig(max_retries=1, initial_delay=0.1, max_delay=5.0),
            timeout_seconds=5,
            bulkhead_config=BulkheadConfig(
                max_concurrent_calls=100, max_waiting_calls=200, name="rate_limiter"
            ),
        ),
    }
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        api_client = ResilientClient("payment_api", timeout_seconds=10)
        ...
        result = await api_client.call(make_payment, 100.0, operation_name="process_payment")
