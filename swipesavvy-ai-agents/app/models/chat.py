...
from sqlalchemy import UUID, Boolean, Column, DateTime, Enum, ForeignKey, Integer, String, Text
...
class ChatParticipant(Base):
    ...
    chat_session_id = Column(
        UUID(as_uuid=True), ForeignKey(CHAT_SESSIONS_ID), nullable=False, index=True
    )
...
class ChatAuditLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    chat_session_id = Column(
        UUID(as_uuid=True), ForeignKey(CHAT_SESSIONS_ID), nullable=True, index=True
    )
    user_id = Column(UUID(as_uuid=True), nullable=False, index=True)
