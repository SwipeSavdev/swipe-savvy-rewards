...
class ChatConnectionManager:
    ...
    async def connect(self, chat_session_id: str, user_id: str, websocket: Any) -> None:
        ...
        try:
            ...
            connection_user = ConnectionUser(
                user_id=user_id, chat_session_id=chat_session_id, connection=websocket
            )
            ...
            if chat_session_id in self.active_connections:
                if user_id in self.active_connections[chat_session_id]:
                    ...
                    if not self.active_connections[chat_session_id]:
                        ...
                        logger.info(
                            f"Chat session {chat_session_id} removed (no active connections)"
                        )
    ...
    async def send_to_user(
        self, chat_session_id: str, user_id: str, message: WebSocketMessage
    ) -> bool:
        """
    ...
    async def broadcast(
        self, chat_session_id: str, message: WebSocketMessage, exclude_user_id: Optional[str] = None
    ) -> Dict[str, bool]:
        """
    ...
    def get_session_users(self, chat_session_id: str) -> List[Dict[str, Any]]:
        ...
        return [user.to_dict() for user in self.active_connections[chat_session_id].values()]
    ...
    def is_user_online(self, chat_session_id: str, user_id: str) -> bool:
        ...
        """
        return (
            chat_session_id in self.active_connections
            and user_id in self.active_connections[chat_session_id]
            and self.active_connections[chat_session_id][user_id].is_active
        )
