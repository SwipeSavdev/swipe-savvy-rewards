...
class FirebaseService:
    ...
    def send_multicast(
        self, user_ids: List[UUID], title: str, body: str, data: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _get_user_device_tokens(self, user_id: UUID) -> List[str]:
        ...
        try:
            devices_ref = (
                self.db.child("devices").order_by_child("user_id").equal_to(str(user_id)).get()
            )
    ...
    def unregister_device(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            self.db.child("devices").child(device_id).update(
                {"is_active": False, "unregistered_at": datetime.utcnow().isoformat()}
            )
...
class NotificationPreferencesService:
    ...
    def set_preferences(self, user_id: UUID, preferences: Dict[str, bool]) -> Dict[str, Any]:
        """
        ...
        """
        try:
            pref_data = {**preferences, "updated_at": datetime.utcnow().isoformat()}
    ...
    def check_notification_allowed(self, user_id: UUID, notification_type: str) -> bool:
        """
