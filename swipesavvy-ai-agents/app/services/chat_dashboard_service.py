...
class ChatDashboardService:
    ...
    def get_session_stats(db: Session, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            response_times = (
                db.query(func.avg(ChatMessage.created_at - ChatSession.created_at))
                .join(ChatSession)
                .filter(ChatSession.created_at >= since)
                .scalar()
            )

            avg_response_time_seconds = response_times.total_seconds() if response_times else 0
    ...
    def get_agent_performance(
        db: Session, agent_id: Optional[UUID] = None, time_range_hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_active_sessions(db: Session, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            sessions = (
                ...
                .filter(
                    ChatSession.status.in_([ChatSessionStatus.ACTIVE, ChatSessionStatus.WAITING])
                )
            ...
            for session in sessions:
                ...
                participants = (
                    ...
                    .filter(ChatParticipant.chat_session_id == session.id)
                    .all()
                )
                ...
                active.append(
                    {
                        ...
                        "assigned_agent_id": (
                            str(session.assigned_agent_id) if session.assigned_agent_id else None
                        ),
                        ...
                        "latest_message": (
                            {
                                ...
                                "sender_id": (
                                    str(latest_message.sender_id) if latest_message else None
                                ),
                            }
                            if latest_message
                            else None
    ...
    def get_waiting_sessions(db: Session) -> List[Dict[str, Any]]:
        ...
        try:
            sessions = (
                db.query(ChatSession)
                .filter(ChatSession.status == ChatSessionStatus.WAITING)
                ...
                waiting.append(
                    {
                        ...
                        "priority": (
                            "high" if wait_time > 300 else "medium" if wait_time > 60 else "low"
                        ),
                    }
                )
    ...
    def get_message_analytics(db: Session, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            text_msgs = (
                db.query(func.count(ChatMessage.id))
                .filter(and_(ChatMessage.created_at >= since, ChatMessage.message_type == "text"))
                .scalar()
                or 0
            )
            ...
            avg_length = (
                ...
                .filter(and_(ChatMessage.created_at >= since, ChatMessage.content.isnot(None)))
                .scalar()
                or 0
            )
            ...
            return {
                "total_messages": total_messages,
                "message_types": {"text": text_msgs, "files": file_msgs},
                "status_distribution": {"sent": sent, "delivered": delivered, "read": read},
    ...
    @staticmethod
    def assign_session_to_agent(db: Session, session_id: UUID, agent_id: UUID) -> bool:
        """
