...
class CampaignBuilder:
    ...
    def create_campaigns_from_behaviors(
    ...
    def _create_vip_campaign(self, behaviors: List[UserBehavior]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "qualifying_criteria": {"min_spend": 5000, "min_transactions": 15},
            "duration_days": 30,
    ...
    def _create_loyalty_campaign(self, behaviors: List[UserBehavior]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "qualifying_criteria": {"min_transactions": 10},
            "duration_days": 60,
            "created_at": datetime.utcnow().isoformat(),
            "status": "active",
        }
    ...
    def _create_location_campaigns(self, behaviors: List[UserBehavior]) -> List[Dict[str, Any]]:
        ...
        campaigns = []
        location_clustered = [
            b for b in behaviors if BehaviorPattern.LOCATION_CLUSTERED in b.patterns
        ]
        ...
        for location, users in location_groups.items():
            if len(users) >= 3:  # Only create if targeting at least 3 users
                campaign = {
                    ...
                    "qualifying_criteria": {"primary_location": location},
                    "duration_days": 30,
                    "created_at": datetime.utcnow().isoformat(),
                    "status": "active",
                }
    ...
    def _create_reengagement_campaign(
        self, behaviors: List[UserBehavior]
        ...
        return {
            ...
            "qualifying_criteria": {"days_since_transaction": 30},
            "duration_days": 14,
            "created_at": datetime.utcnow().isoformat(),
            "status": "active",
        }
    ...
    def _create_welcome_campaign(self, behaviors: List[UserBehavior]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "qualifying_criteria": {"transaction_count_max": 5},
            "duration_days": 21,
            "created_at": datetime.utcnow().isoformat(),
            "status": "active",
        }

    def _create_spending_milestone_campaign(
        self, behaviors: List[UserBehavior]
    ) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "qualifying_criteria": {"spending_threshold": 1000},
            "duration_days": 90,
            "created_at": datetime.utcnow().isoformat(),
            "status": "active",
        }
...
class UserSegmentationEngine:
    ...
    def segment_users_for_campaign(
        self, campaign: Dict[str, Any], behaviors: List[UserBehavior]
    ) -> List[str]:
        """
    ...
    def _matches_criteria(
        self, behavior: UserBehavior, criteria: Dict[str, Any], pattern: str
    ) -> bool:
        ...
        try:
            ...
            for campaign in campaigns:
                try:
                    ...
                    logger.info(
                        f"Created campaign '{campaign['name']}' targeting {len(target_users)} users"
                    )
        ...
        try:
            ...
            if campaign.get("target_user_ids"):
                for user_id in campaign["target_user_ids"]:
                    ...
                    """
                    cursor.execute(
                        target_query, (campaign_id, user_id, "eligible", datetime.utcnow())
                    )

            self.conn.commit()
            logger.info(
                f"Saved campaign {campaign_id} with {len(campaign.get('target_user_ids', []))} targets"
            )
