...
MOCK_EMAIL = (
    os.getenv("MOCK_EMAIL", "true" if ENVIRONMENT == "development" else "false").lower() == "true"
)
...
class AWSSESService:
        ...
        if not self.mock_mode:
            try:
                ...
                if AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY:
                    ...
                    logger.info(
                        f"AWS SES service initialized with explicit credentials in region: {AWS_REGION}"
                    )
                else:
                    # ECS/Lambda: Use IAM role credentials automatically
                    self.client = boto3.client("ses", region_name=AWS_REGION)
                    logger.info(
                        f"AWS SES service initialized with IAM role in region: {AWS_REGION}"
                    )
    ...
    async def send_kyc_status_email(
        ...
        if status == "approved":
            ...
            message = (
                "We're currently reviewing your documents. This usually takes 1-2 business days."
            )
    ...
    async def send_template_email(
        ...
        template_handlers = {
            ...
            "welcome": lambda: self.send_welcome_email(
                to_email, template_data.get("user_name", "User")
            ),
    ...
    async def send_user_invitation(
        self, to_email: str, name: str, invite_link: str, inviter_name: str = "SwipeSavvy"
    ) -> bool:
        """Send user invitation email using template"""
        template = EmailTemplates.user_invitation(name, invite_link, inviter_name)
        return await self.send_email(
            to_email, template["subject"], template["html_body"], template["text_body"]
        )

    async def send_email_verification_with_code(
        self, to_email: str, name: str, verification_code: str, verification_link: str
    ) -> bool:
    ...
    async def send_password_reset_with_code(
        self, to_email: str, name: str, reset_code: str, reset_link: str
    ) -> bool:
    ...
    async def send_kyc_approved_email(self, to_email: str, name: str, tier: str = "Tier 2") -> bool:
    ...
    async def send_kyc_rejected_email(
        self, to_email: str, name: str, reason: str, can_retry: bool = True
    ) -> bool:
    ...
    async def send_cashback_earned_email(
        self, to_email: str, name: str, amount: float, merchant: str, total_balance: float
    ) -> bool:
    ...
    async def send_withdrawal_initiated_email(
        self, to_email: str, name: str, amount: float, method: str, estimated_arrival: str
    ) -> bool:
    ...
    async def send_withdrawal_completed_email(
        self, to_email: str, name: str, amount: float, method: str
    ) -> bool:
    ...
    async def send_new_device_login_email(
        self, to_email: str, name: str, device: str, location: str, ip_address: str, timestamp: str
    ) -> bool:
    ...
    async def send_suspicious_activity_email(
        self, to_email: str, name: str, activity: str, recommendation: str
    ) -> bool:
    ...
    async def send_login_otp_email(
        self, to_email: str, otp_code: str, user_name: str = "User"
    ) -> bool:
...
async def send_contact_form_notification(to_email: str, submission_data: Dict[str, Any]) -> bool:
...
async def send_demo_request_notification(to_email: str, submission_data: Dict[str, Any]) -> bool:
