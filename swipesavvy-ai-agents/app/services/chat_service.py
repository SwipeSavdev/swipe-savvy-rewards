...
class ChatService:
    ...
    def create_message(
        ...
        try:
            ...
            logger.info(
                f"Message {message.id} created in session {chat_session_id} " f"by user {sender_id}"
            )
    ...
    def get_session_messages(
        ...
        """
        try:
            query = db.query(ChatMessage).filter(ChatMessage.chat_session_id == chat_session_id)

            if exclude_deleted:
                query = query.filter(ChatMessage.deleted_at.is_(None))

            messages = (
                query.order_by(desc(ChatMessage.created_at)).limit(limit).offset(offset).all()
            )
    ...
    @staticmethod
    def update_message_status(db: Session, message_id: UUID, status: ChatMessageStatus) -> bool:
        """
    ...
    def create_session(
        ...
        try:
            session = ChatSession(
                ...
                metadata=metadata,
                status=(
                    ChatSessionStatus.WAITING if not assigned_agent_id else ChatSessionStatus.ACTIVE
                ),
            )
            ...
            participant = ChatParticipant(
                chat_session_id=session.id, user_id=initiator_id, role=ChatParticipantRole.USER
            )
            db.add(participant)
    ...
    @staticmethod
    def close_session(
        db: Session, session_id: UUID, rating: Optional[int] = None, feedback: Optional[str] = None
    ) -> bool:
        """
    ...
    @staticmethod
    def get_participant(db: Session, session_id: UUID, user_id: UUID) -> Optional[ChatParticipant]:
        """
    ...
    def add_participant(
        ...
        try:
            ...
            participant = ChatParticipant(chat_session_id=session_id, user_id=user_id, role=role)
    ...
    def get_session_participants(db: Session, session_id: UUID) -> List[ChatParticipant]:
        ...
        return (
            ...
            .filter(
                and_(
                    ChatParticipant.chat_session_id == session_id, ChatParticipant.is_active == True
                )
            )
            .all()
        )
    ...
    @staticmethod
    def mark_read(db: Session, session_id: UUID, user_id: UUID, up_to_message_id: UUID) -> bool:
        """
        ...
        try:
            ...
            message = db.query(ChatMessage).filter(ChatMessage.id == up_to_message_id).first()
            if not message:
                return False
    ...
    @staticmethod
    def block_user(
        db: Session, user_id: UUID, blocked_user_id: UUID, reason: Optional[str] = None
    ) -> ChatBlockedUser:
        """
        ...
        try:
            ...
            blocked = ChatBlockedUser(
                user_id=user_id, blocked_user_id=blocked_user_id, reason=reason
            )
