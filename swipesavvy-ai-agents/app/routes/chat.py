...
async def websocket_endpoint(
    websocket: WebSocket, chat_session_id: UUID, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        try:
            ...
            if not session:
                await websocket.close(
                    code=status.WS_1008_POLICY_VIOLATION, reason=SESSION_NOT_FOUND
                )
                return
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if message_data.message_type == "typing":
                        try:
                            ...
                            is_typing = (
                                message_data.content == "true" if message_data.content else False
                            )
                            await manager.mark_typing(connection_id, str(user_id), is_typing)
                        except Exception as e:
                            logger.error(f"Error marking typing: {str(e)}")
                            await websocket.send_json(
                                {"success": False, "message": "Error processing typing indicator"}
                            )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Message sent: {db_message.id} in session {chat_session_id}"
                            )
                        except Exception as e:
                ...
                try:
                    await websocket.send_json(
                        {"success": False, "message": "Server error processing message"}
                    )
                except RuntimeError:
                    pass  # Connection might be closed
...
async def get_session(
    session_id: UUID, current_user: str = Depends(verify_jwt_token), db: Session = Depends(get_db)
):
...
async def mark_read(
    ...
    return ChatResponse(
        success=True, message="Messages marked as read", data={"session_id": str(session_id)}
    )
...
async def block_user(
    ...
    ChatService.block_user(
        db=db, user_id=current_user, blocked_user_id=request.blocked_user_id, reason=request.reason
    )
    db.commit()
