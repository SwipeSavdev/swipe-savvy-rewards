...
async def get_agent_performance(
    ...
    """
    try:
        performance = ChatDashboardService.get_agent_performance(db, agent_id, time_range_hours)
        return performance
...
async def assign_session_to_agent(
    ...
    """
    try:
        success = ChatDashboardService.assign_session_to_agent(db, session_id, request.agent_id)
        db.commit()
...
async def broadcast_active_sessions_update(db: Session):
    ...
    try:
        sessions = ChatDashboardService.get_active_sessions(db, 24, 100)
        await dashboard_connections.broadcast(
            {"type": "session_update", "data": sessions, "timestamp": datetime.utcnow().isoformat()}
        )
    except Exception as e:
...
async def dashboard_health_check():
    ...
    return {"status": "healthy", "service": "chat-dashboard", "timestamp": ""}
