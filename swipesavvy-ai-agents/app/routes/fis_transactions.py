...
async def get_transactions(
    ...
    response = await tx_service.get_transactions(
        card_id=card_id, filters=filters, pagination=pagination
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get transactions"
        )
...
async def get_recent_transactions(
    ...
    verify_card_ownership(card_id, user_id)
    response = await tx_service.get_recent_transactions(card_id=card_id, limit=limit)

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get recent transactions"
        )
...
async def get_pending_transactions(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get pending transactions"
        )
...
async def get_transaction_summary(
    ...
    response = await tx_service.get_transaction_summary(
        card_id=card_id, start_date=start_date, end_date=end_date
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get summary"
        )
...
async def get_spending_by_category(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get category breakdown"
        )
...
async def get_spending_by_merchant(
    ...
    response = await tx_service.get_spending_by_merchant(
        card_id=card_id, start_date=start_date, end_date=end_date, limit=limit
    )
...
async def initiate_dispute(
    ...
    response = await tx_service.initiate_dispute(
        ...
        expected_credit_amount=(
            Decimal(str(request.expected_credit_amount)) if request.expected_credit_amount else None
        ),
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to initiate dispute"
        )

    return {"success": True, "message": "Dispute initiated", "data": response.data}
...
async def get_disputes(
    ...
    verify_card_ownership(card_id, user_id)
    response = await tx_service.get_disputes(card_id=card_id, status=status)

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get disputes"
        )
...
async def get_dispute(
    ...
    verify_card_ownership(card_id, user_id)
    response = await tx_service.get_dispute(card_id=card_id, dispute_id=dispute_id)

    if not response.success:
        raise HTTPException(status_code=404, detail=response.error_message or "Dispute not found")
...
async def add_dispute_document(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to add document"
        )

    return {"success": True, "message": "Document added to dispute"}
...
async def add_transaction_note(
    ...
    response = await tx_service.add_transaction_note(
        card_id=card_id, transaction_id=transaction_id, note=request.note
    )

    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to add note")
...
async def categorize_transaction(
    ...
    response = await tx_service.categorize_transaction(
        card_id=card_id, transaction_id=transaction_id, category=request.category
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to categorize transaction"
        )

    return {"success": True, "message": "Transaction categorized"}
