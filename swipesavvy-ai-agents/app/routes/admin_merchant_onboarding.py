...
router = APIRouter(
    ...
    tags=["admin-merchant-onboarding"],
...
async def aba_lookup(routing_number: str) -> ABALookupResponse:
    ...
    try:
        ...
        return ABALookupResponse(routing_number=routing_number, bank_name="", valid=False)
...
async def get_onboarding(merchant_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        onboarding = (
            db.query(OnboardingModel).filter(OnboardingModel.merchant_id == merchant_id).first()
        )

        if not onboarding:
            return {"success": True, "has_onboarding": False, "onboarding": None}
...
async def start_onboarding(merchant_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
...
async def save_onboarding(
    merchant_id: str, request: OnboardingSaveRequest, db: Session = Depends(get_db)
) -> Dict[str, Any]:
    ...
    try:
        ...
        if request.business_info:
            ...
            onboarding.customer_service_phone = bi.get(
                "customer_service_phone", onboarding.customer_service_phone
            )
            if "address" in bi:
                addr = bi["address"]
...
async def upload_document(
    ...
    try:
        ...
        uploads_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "uploads", "onboarding", str(merchant_id)
        )
        ...
        return {
            ...
            "document": {"id": doc_id, "type": doc_type, "filename": file.filename},
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def submit_to_fiserv(merchant_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        # Submit to Fiserv
        result = await fiserv.submit_mpa(xml_content=xml_content, ext_ref_id=onboarding.ext_ref_id)
...
async def check_fiserv_status(merchant_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        result = await fiserv.get_status_by_mpa_id(
            mpa_id=onboarding.mpa_id, ext_ref_id=onboarding.ext_ref_id
        )
...
async def delete_onboarding(merchant_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        if onboarding.status != "draft":
            raise HTTPException(
                status_code=422, detail="Cannot delete onboarding that has been submitted"
            )

        db.delete(onboarding)
        db.commit()

        return {"success": True, "message": "Onboarding deleted"}
    except HTTPException:
        raise
    except Exception as e:
