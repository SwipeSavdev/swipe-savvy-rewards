...
async def get_dashboard_overview(
    ...
    try:
        ...
        transaction_count = (
            db.execute(text("SELECT COUNT(*) FROM wallet_transactions")).scalar() or 0
        )
        total_revenue = (
            db.execute(text("SELECT COALESCE(SUM(amount), 0) FROM wallet_transactions")).scalar()
            or 0
        )

        # Create stats with real data
        stats = DashboardStats(
            users=StatMetric(value=total_users, trendPct=2.5, trendDirection="up"),  # Placeholder
            transactions=StatMetric(
                value=transaction_count, trendPct=1.2, trendDirection="up"  # Placeholder
            ),
            revenue=StatMetric(
                value=int(total_revenue), trendPct=0.8, trendDirection="up"  # Placeholder
            ),
            growth=StatMetric(value=1.5, trendPct=0.5, trendDirection="up"),  # Placeholder
        )
...
async def get_analytics_overview(
    ...
    try:
        ...
        return AnalyticsOverview(
            activeUsers=0, transactions=0, revenue=0, conversion=0.0, trends={}
        )


@router.get("/analytics/transactions")
async def get_transactions_chart(
    days: int = 30, current_user: str = Depends(verify_jwt_token), db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        for i in range(days):
            ...
            day_transactions = (
                ...
                .filter(func.date(WalletTransaction.created_at) == date.date())
                .count()
            )
        ...
        return {"title": "Transactions Volume", "subtitle": f"Last {days} days", "data": data}
    except Exception as e:
        logger.error(f"Error getting transactions chart: {str(e)}")
        return {"title": "Transactions Volume", "subtitle": f"Last {days} days", "data": []}
    ...
    **Query params:**
    ...
    try:
        ...
        for i in range(days):
            ...
            data.append({"date": date_str, "revenue": int(float(day_revenue))})

        return {"title": "Revenue", "subtitle": f"Last {days} days", "data": data}
    except Exception as e:
        logger.error(f"Error getting revenue chart: {str(e)}")
        return {"title": "Revenue", "subtitle": f"Last {days} days", "data": []}
        ...
        return {
            "funnel": [
                {"stage": "signup", "label": "Started", "count": total_users, "percentage": 100},
                {
                    ...
                    "count": int(total_users * 0.87),
                    "percentage": 87,
                },
                {
                    "stage": "kyc_started",
                    "label": "KYC Started",
                    "count": int(total_users * 0.72),
                    "percentage": 72,
                },
                {
                    "stage": "kyc_completed",
                    "label": "KYC Completed",
                    "count": int(total_users * 0.68),
                    "percentage": 68,
                },
                {
                    "stage": "payment_method",
                    "label": "Added Payment Method",
                    "count": int(total_users * 0.54),
                    "percentage": 54,
                },
                {
...
async def get_support_stats(
    ...
    try:
        ...
        resolved_today = (
            ...
            .filter((SupportTicket.status == "closed") & (SupportTicket.updated_at >= today))
            .count()
        )
        ...
        return {
            ...
            "slaMetrics": {"firstResponseSLA": 87.5, "resolutionSLA": 75.2, "csat": 4.6},
        }
    except Exception as e:
...
async def seed_sample_data(
    ...
    default_password_hash = (
        "$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/X4UaaD3NJnwB6H7/."  # hash of "Demo123"
    )
    ...
    try:
        ...
        for i, (name, category, biz_type) in enumerate(merchant_categories):
            ...
            existing = db.execute(
                text("SELECT id FROM merchants WHERE email = :email"), {"email": email}
            ).fetchone()
            if existing:
