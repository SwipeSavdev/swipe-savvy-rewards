...
router = APIRouter(
    prefix="/api/v1/ai-concierge", tags=["ai-concierge"], dependencies=[Depends(require_auth)]
)
...
class AIConciergeRequest(BaseModel):
    ...
    role: Optional[str] = Field(
        "analyst", description="User role: super_admin, admin, support, analyst"
    )
    permissions: Optional[List[str]] = Field(
        None, description="List of user permissions for fine-grained access control"
    )
    employee_name: Optional[str] = Field(
        None, description="Employee name for personalized responses"
    )
...
async def ai_concierge_chat(request: AIConciergeRequest):
    ...
    async def generate_response_stream():
        ...
        try:
            ...
            if request.context:
                ...
                if request.context.get("current_page"):
                    context_parts.append(
                        f"Employee is currently on: {request.context['current_page']}"
                    )
                if request.context.get("selected_item"):
                    context_parts.append(
                        f"Employee has selected: {request.context['selected_item']}"
                    )
                if context_parts:
                    additional_context = "\n".join(context_parts)
            ...
            logger.info(
                f"AI Concierge chat for {role} (max txn: ${max_amount if max_amount else 'unlimited'})"
            )
...
async def agentic_concierge_chat(request: AgenticChatRequest):
    ...
    return StreamingResponse(generate_agentic_stream(), media_type="text/event-stream")
