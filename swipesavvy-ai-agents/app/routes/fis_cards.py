...
async def issue_virtual_card(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to issue virtual card"
        )

    return {"success": True, "message": "Virtual card issued successfully", "data": response.data}
...
async def order_physical_card(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to order physical card"
        )

    return {"success": True, "message": "Physical card ordered successfully", "data": response.data}
...
async def get_user_cards(
    ...
    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to get cards")
...
async def get_card(
    ...
    if not response.success:
        raise HTTPException(status_code=404, detail=response.error_message or "Card not found")
...
async def get_card_sensitive_data(
    ...
    verification_code: Optional[str] = Query(
        None, description="PIN or OTP for step-up authentication"
    ),
    user_id: str = Depends(require_auth),
    card_service: FISCardService = Depends(get_fis_card_service),
    ...
    if not verification_code:
        raise HTTPException(
            status_code=403, detail="Step-up authentication required to view card details"
        )

    # Validate the PIN/verification code against FIS
    pin_result = await pin_service.validate_pin(
        card_id=card_id, pin=verification_code, operation="view_sensitive_data"
    )
    if not pin_result.success:
        logger.warning(
            f"Step-up auth failed for sensitive data access: " f"user={user_id}, card={card_id}"
        )
        raise HTTPException(
            status_code=403,
    ...
    response = await card_service.get_card_sensitive_data(
        card_id=card_id, include_pan=include_pan, include_cvv=include_cvv
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get sensitive data"
        )
...
async def activate_card(
    ...
    response = await card_service.activate_card(
        card_id=card_id, last_four=request.last_four, activation_code=request.activation_code
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to activate card"
        )

    return {"success": True, "message": "Card activated successfully", "data": response.data}
...
async def replace_card(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to replace card"
        )

    return {"success": True, "message": "Card replacement ordered", "data": response.data}
...
async def cancel_card(
    ...
    verify_card_ownership(card_id, user_id)
    response = await card_service.cancel_card(card_id=card_id, reason=request.reason)

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to cancel card"
        )
...
async def get_shipping_status(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get shipping status"
        )
...
async def lock_card(
    ...
    verify_card_ownership(card_id, user_id)
    response = await controls_service.lock_card(card_id=card_id, reason=request.reason)

    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to lock card")
...
async def unlock_card(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to unlock card"
        )
...
async def freeze_card(
    ...
    response = await controls_service.freeze_card(
        card_id=card_id, reason=request.reason or "user_request"
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to freeze card"
        )
...
async def unfreeze_card(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to unfreeze card"
        )
...
async def get_all_controls(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get controls"
        )
...
async def get_spending_limits(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get limits"
        )
...
async def set_spending_limits(
    ...
    limits = SpendingLimits(
        ...
        per_transaction_limit=(
            Decimal(str(request.per_transaction_limit)) if request.per_transaction_limit else None
        ),
    )
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to set limits"
        )
...
async def remove_spending_limits(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to remove limits"
        )

    return {"success": True, "message": "Spending limits removed"}
...
async def set_channel_controls(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to set channel controls"
        )

    return {"success": True, "message": "Channel controls updated"}
...
async def enable_international(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to enable international"
        )

    return {"success": True, "message": "International transactions enabled"}
...
async def disable_international(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to disable international"
        )

    return {"success": True, "message": "International transactions disabled"}
...
async def set_merchant_controls(
    ...
    controls = MerchantControls(
        blocked_mcc_codes=request.blocked_mcc_codes, allowed_mcc_codes=request.allowed_mcc_codes
    )
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to set merchant controls"
        )

    return {"success": True, "message": "Merchant controls updated"}
...
async def block_merchant_category(
    ...
    response = await controls_service.block_merchant_category(
        card_id=card_id, category=request.category
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to block category"
        )

    return {"success": True, "message": f"Category '{request.category}' blocked"}
...
async def unblock_merchant_category(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to unblock category"
        )

    return {"success": True, "message": f"Category '{request.category}' unblocked"}
...
async def set_geo_controls(
    ...
    controls = GeoControls(
        allowed_countries=request.allowed_countries, blocked_countries=request.blocked_countries
    )
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to set geo controls"
        )

    return {"success": True, "message": "Geographic controls updated"}
...
async def block_country(
    ...
    response = await controls_service.block_country(
        card_id=card_id, country_code=request.country_code
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to block country"
        )

    return {"success": True, "message": f"Country '{request.country_code}' blocked"}
...
async def unblock_country(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to unblock country"
        )

    return {"success": True, "message": f"Country '{request.country_code}' unblocked"}
...
async def set_alert_preferences(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to set alert preferences"
        )
...
async def set_pin(
    ...
    """Set initial PIN."""
    verify_card_ownership(card_id, user_id)
    response = await pin_service.set_pin(card_id=card_id, pin=request.pin)

    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to set PIN")
...
async def change_pin(
    ...
    response = await pin_service.change_pin(
        card_id=card_id, current_pin=request.current_pin, new_pin=request.new_pin
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to change PIN"
        )
...
async def reset_pin(
    ...
    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to reset PIN")
...
async def validate_pin(
    ...
    response = await pin_service.validate_pin(
        card_id=card_id, pin=request.pin, operation=request.operation
    )

    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Invalid PIN")
...
async def get_pin_status(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get PIN status"
        )
...
async def unlock_pin(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to unlock PIN"
        )
...
async def request_pin_reset_otp(
    ...
    if not response.success:
        raise HTTPException(status_code=400, detail=response.error_message or "Failed to send OTP")
