...
class SendEventNotificationRequest(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Event type: payment, campaign, support, security, feature"
    )
    ...
    **Returns:**
    ...
    try:
        ...
        return NotificationResponse(
            success=True, message="Device registered successfully", data=result
        )
...
async def unregister_device(
    device_id: str, user_id: UUID = Depends(verify_jwt_token), db: Session = Depends(get_db)
):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        return NotificationResponse(
            success=True, message="Device unregistered successfully", data=result
        )
...
async def update_preferences(
    ...
    **Returns:**
    ...
    try:
        ...
        return NotificationResponse(
            success=True, message="Preferences updated successfully", data=result
        )
...
async def get_preferences(user_id: UUID = Depends(verify_jwt_token), db: Session = Depends(get_db)):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        return NotificationResponse(
            ...
            data={"user_id": str(user_id), "preferences": prefs},
        )
    ...
    **Returns:**
    ...
    try:
        ...
        if notification_type and notification_type not in valid_types:
            raise HTTPException(
                status_code=400, detail=f"Invalid notification_type. Must be one of: {valid_types}"
            )
        ...
        logger.info(
            f"Notification history retrieved for user {user_id}: {len(notifications)} records"
        )
    ...
    **Returns:**
    ...
    try:
        ...
        result = firebase_service.send_notification(
            user_id=user_id, title=request.title, body=request.body, data=request.data
        )
        ...
        return NotificationResponse(
            success=result.get("success", True), message=message, data=result
        )
...
async def mark_notification_as_read(
    notification_id: str, user_id: UUID = Depends(verify_jwt_token), db: Session = Depends(get_db)
):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        return NotificationResponse(
            ...
            data={"notification_id": notification_id, "read_at": datetime.utcnow().isoformat()},
        )
    ...
    **Returns:**
    ...
    try:
        ...
        result = firebase_service.send_event_notification(
            user_id=user_id, event_type=request.event_type, event_data=request.event_data or {}
        )
...
async def sns_register_device(
    request: SNSDeviceRegistrationRequest, current_user: dict = Depends(verify_jwt_token)
):
    """
    ...
    if not sns_service:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="SNS service not configured"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Device registration failed"
        )


@router.post("/sns/send")
async def sns_send_notification(
    request: SNSSendToEndpointRequest, current_user: dict = Depends(verify_jwt_token)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to send notification"
        )


@router.delete("/sns/unregister/{endpoint_arn}")
async def sns_unregister_device(endpoint_arn: str, current_user: dict = Depends(verify_jwt_token)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to unregister device"
        )


@router.get("/sns/status/{endpoint_arn}")
async def sns_get_endpoint_status(
    endpoint_arn: str, current_user: dict = Depends(verify_jwt_token)
):
    """
    ...
    try:
        ...
        return {"endpoint_arn": decoded_arn, "attributes": attributes}
