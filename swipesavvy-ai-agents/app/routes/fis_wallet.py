...
async def get_apple_pay_eligibility(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to check eligibility"
        )
...
async def provision_apple_pay(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to provision Apple Pay"
        )

    return {"success": True, "message": "Apple Pay provisioned successfully", "data": response.data}
...
async def provision_google_pay(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to provision Google Pay"
        )
...
async def get_google_pay_push_token(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get push token"
        )
...
async def provision_samsung_pay(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to provision Samsung Pay"
        )
...
async def get_wallet_tokens(
    ...
    response = await wallet_service.get_wallet_tokens(
        card_id=card_id, wallet_type=parsed_wallet_type, status=parsed_status
    )

    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get tokens"
        )
...
async def get_token(
    ...
    if not response.success:
        raise HTTPException(status_code=404, detail=response.error_message or "Token not found")
...
async def suspend_token(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to suspend token"
        )

    return {"success": True, "message": "Token suspended"}
...
async def resume_token(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to resume token"
        )

    return {"success": True, "message": "Token resumed"}
...
async def delete_token(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to delete token"
        )
...
async def suspend_all_tokens(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to suspend tokens"
        )

    return {"success": True, "message": "All tokens suspended"}
...
async def delete_all_tokens(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to delete tokens"
        )

    return {"success": True, "message": "All tokens deleted"}
...
async def get_token_activity(
    ...
    if not response.success:
        raise HTTPException(
            status_code=400, detail=response.error_message or "Failed to get token activity"
        )
