...
STRUCTURED_LOG_FORMAT = {
    ...
    "status": "success",
    "error": None,
    "context": {"database": "ok", "cache": "ok", "external_api": "ok"},
}
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(operation_name, duration_ms=int(duration_ms), status="success")
                return result
            except Exception as e:
                duration_ms = (time.time() - start_time) * 1000
                logger.error(
                    operation_name, error=e, duration_ms=int(duration_ms), status="failure"
                )
                raise
