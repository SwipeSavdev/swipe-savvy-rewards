...
def ingest_to_database(documents: List[Dict[str, Any]], db_url: str):
    ...
    cur.execute(
        "SELECT column_name, data_type FROM information_schema.columns WHERE table_name='kb_chunks' AND column_name='embedding'"
    )
    result = cur.fetchone()
    if result:
        ...
        if current_dim != emb_service.dimension:
            ...
            cur.execute(
                f"ALTER TABLE kb_chunks ALTER COLUMN embedding TYPE vector({emb_service.dimension})"
            )
            conn.commit()
    ...
    for doc_idx, doc in enumerate(documents, 1):
        ...
        for chunk_idx, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            ...
            chunk_data.append(
                (doc_id, chunk_idx, chunk, embedding, token_count, psycopg2.extras.Json({}))
            )
