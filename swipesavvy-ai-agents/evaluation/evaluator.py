...
class ConciergeEvaluator:
    ...
    async def run_test_case(self, test_case: Dict[str, Any]) -> EvaluationResult:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return self._create_error_result(
                    test_id, category, user_input, gold_response, expected, errors, latency_ms
                )
            ...
            return self._create_error_result(
                test_id, category, user_input, gold_response, expected, errors, 0
            )
        ...
        if not tools_called_correct:
            warnings.append(f"Tool mismatch: expected {expected_tools}, got {actual_tools}")

        # Evaluate response quality
        expected_keywords = expected.get("response_contains", [])
        missing_keywords = [
            kw for kw in expected_keywords if kw.lower() not in actual_response.lower()
        ]
        ...
        if expected_handoff and actual_handoff:
            ...
            if expected_reason:
                ...
                if not handoff_reason_correct:
                    warnings.append(
                        f"Handoff reason: expected '{expected_reason}', " f"got '{actual_reason}'"
                    )

        if not handoff_correct:
            errors.append(
                f"Handoff mismatch: expected {expected_handoff}, " f"got {actual_handoff}"
            )

        # Check latency
        max_latency = expected.get("latency_max_ms", 5000)
        if latency_ms > max_latency:
            warnings.append(f"Latency {latency_ms}ms exceeds max {max_latency}ms")

        # Calculate response quality score (1-5)
        quality_score = self._calculate_quality_score(
            actual_response, gold_response, contains_expected, tools_called_correct, handoff_correct
        )

        # Calculate overall score
        score = self._calculate_test_score(
            tools_called_correct,
            tools_order_correct,
            contains_expected,
            handoff_correct,
            quality_score,
        ...
        passed = handoff_correct and quality_score >= min_quality and len(errors) == 0
    ...
    def _calculate_quality_score(
        self, actual: str, gold: str, has_keywords: bool, tools_correct: bool, handoff_correct: bool
    ) -> int:
        """Calculate response quality score (1-5)"""
    ...
    def _calculate_test_score(
        ...
        weights = {"tools": 0.25, "order": 0.10, "keywords": 0.20, "handoff": 0.25, "quality": 0.20}
    ...
    def _generate_summary(self, results: List[EvaluationResult]) -> EvaluationSummary:
        ...
        true_positives = sum(1 for r in results if r.expected_handoff and r.actual_handoff)
        false_positives = sum(1 for r in results if not r.expected_handoff and r.actual_handoff)
        false_negatives = sum(1 for r in results if r.expected_handoff and not r.actual_handoff)
    ...
    def _generate_report(self, summary: EvaluationSummary, results: List[EvaluationResult]) -> str:
