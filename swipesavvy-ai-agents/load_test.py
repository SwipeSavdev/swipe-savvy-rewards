...
class LoadTester:
    ...
    async def test_concurrent_requests(
        ...
        logger.info(
            f"✓ {test_name} completed: {result.successful_requests}/{total_requests} successful"
        )
    ...
    async def test_connection_pool_stress(
        self, num_connections: int = 60, test_name: str = "connection_pool_stress"
    ) -> LoadTestResult:
        """
        ...
        connection_pool = psycopg2.pool.ThreadedConnectionPool(
            minconn=5, maxconn=num_connections, **db_config
        )
...
async def run_load_tests():
    ...
    await tester.test_connection_pool_stress(
        num_connections=60, test_name="connection_pool_stress_60"
    )

    # Test 4: Spike load
    await tester.test_spike_load(
        endpoint="/health", spike_size=1000, spike_duration=5, test_name="spike_load_1000_requests"
    )
