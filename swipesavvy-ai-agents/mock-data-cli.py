...
def cmd_load(args):
    ...
    merchant_csv = (
        args.merchant_file or "/Users/macbookpro/Documents/Mock Data/MerchantList-North.csv"
    )
...
def cmd_stats(args):
    ...
    try:
        ...
        for category, data in sorted(stats.get("by_category", {}).items()):
            ...
            print(
                f"  {category:15} {data['count']:>8,} transactions   ${data['total']:>12,.2f}  ({pct:>5.1f}%)"
            )
...
def cmd_validate(args):
    ...
    try:
        ...
        checks.append(
            ("Null transaction IDs", result == 0, f"Found {result}" if result > 0 else "OK")
        )
        ...
        result = cursor.fetchone()[0]
        checks.append(
            ("Invalid amounts (<=0)", result == 0, f"Found {result}" if result > 0 else "OK")
        )
        ...
        result = cursor.fetchone()[0]
        checks.append(
            ("Categories present", result > 0, f"Found {result}" if result > 0 else "None")
        )
...
Examples:
  ...
  %(prog)s reset         # Clear and reload default data
    ...
    load_parser.add_argument(
        "--users", type=int, default=100, help="Number of mock users (default: 100)"
    )
    load_parser.add_argument(
        "--transactions", type=int, default=50, help="Transactions per user (default: 50)"
    )
