...
async def get_spending_analytics(user_id: str, period: str = "month"):
    ...
    period_transactions = [
        ...
        and datetime.fromisoformat(t.get("created_at", "").replace("Z", "+00:00"))
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(
                ...
                json={"query": message, "user_id": user_id, "top_k": 3},
            )
...
async def generate_response_stream(
    message: str, user_id: str, session_id: str, context: Optional[dict] = None
...
async def chat(request: ChatRequest):
    ...
    return StreamingResponse(
        generate_response_stream(request.message, request.user_id, session_id, request.context),
...
async def get_conversation(session_id: str):
    ...
    conversation = conversations.get(session_id, [])
    return {"session_id": session_id, "messages": conversation, "message_count": len(conversation)}
