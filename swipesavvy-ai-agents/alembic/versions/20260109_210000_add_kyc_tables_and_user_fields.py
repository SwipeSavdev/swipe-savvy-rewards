...
def upgrade() -> None:
    ...
    # KYC tier and status
    op.add_column(
        "users", sa.Column("kyc_tier", sa.String(20), nullable=False, server_default="tier1")
    )
    op.add_column(
        "users", sa.Column("kyc_status", sa.String(50), nullable=False, server_default="pending")
    )
    ...
    op.create_index(
        "ix_ofac_screening_results_risk_level", "ofac_screening_results", ["risk_level"]
    )
    op.create_index(
        "ix_ofac_screening_results_created_at", "ofac_screening_results", ["created_at"]
    )
