...
class SensitiveDataFilter(logging.Filter):
    ...
    REDACTION_PATTERNS = [
        ...
        (
            r'(?:database_url|db_url|DATABASE_URL)["\'\s=:]+postgresql://([^/]+)@([^/]+)',
            "DATABASE_URL",
        ),
        ...
        try:
            ...
            if record.args:
                if isinstance(record.args, dict):
                    record.args = {
                        k: self._redact(str(v)) if v else v for k, v in record.args.items()
                    }
                elif isinstance(record.args, tuple):
                    record.args = tuple(
                        self._redact(str(arg)) if arg else arg for arg in record.args
                    )
...
class StructuredLogger:
    ...
    def api_call(self, method: str, endpoint: str, status_code: int, duration_ms: float, **kwargs):
        """Log API call"""
        self._log(
            "INFO",
    ...
    def llm_call(
        self, model: str, prompt_tokens: int, completion_tokens: int, duration_ms: float, **kwargs
    ):
        """Log LLM API call"""
        self._log(
            "INFO",
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.metric(
                    f"{operation}_duration", duration_ms, "ms", operation=operation, success=True
                )
                return result
            except Exception as e:
                duration_ms = (time.time() - start_time) * 1000
                logger.error(
                    f"{operation} failed", error=e, operation=operation, duration_ms=duration_ms
                )
                raise
