...
def test_concierge_chat() -> bool:
    ...
    try:
        payload = {"message": "Hello, what's my balance?", "user_id": "test_user_smoke_001"}
        response = requests.post(f"{BASE_URL}/api/v1/chat", json=payload, timeout=TIMEOUT)
...
def test_rag_search() -> bool:
    ...
    try:
        payload = {"query": "How do I transfer money?", "top_k": 3}
        response = requests.post(f"{RAG_URL}/search", json=payload, timeout=TIMEOUT)
...
def test_guardrails_content_safety() -> bool:
    ...
    try:
        payload = {"text": "This is a safe message for testing"}
        response = requests.post(
            f"{GUARDRAILS_URL}/check/content-safety", json=payload, timeout=TIMEOUT
        )
...
def test_guardrails_pii_detection() -> bool:
    """Test PII detection"""
    try:
        payload = {"text": "Hello, this is a test message without PII"}
        response = requests.post(f"{GUARDRAILS_URL}/check/pii", json=payload, timeout=TIMEOUT)
