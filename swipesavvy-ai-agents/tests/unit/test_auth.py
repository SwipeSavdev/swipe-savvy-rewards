...
class TestTokenVerification:
    ...
    def test_verify_invalid_token_format_raises_error(self):
        ...
        with pytest.raises(jwt.InvalidTokenError):
            jwt.decode(
                "invalid.token.format", settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM]
            )
    ...
    def test_verify_malformed_token_raises_error(self):
        ...
        for bad_token in malformed_tokens:
            with pytest.raises(Exception):  # Could be various JWT errors
                jwt.decode(bad_token, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM])
