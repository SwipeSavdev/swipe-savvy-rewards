...
class TestE2EUserJourneys:
    ...
    def session_data(self):
        """Create test session"""
        return {"session_id": f"e2e_test_{int(time.time())}", "user_id": "test_user_12345"}
    ...
    async def test_money_transfer_journey(self, session_data):
        ...
        async with httpx.AsyncClient() as client:
            ...
            assert any(
                word in message_lower for word in ["transfer", "sent", "complete", "confirm"]
            )
    ...
    async def test_rag_service_directly(self):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={"query": "How to transfer money", "limit": 5},
                timeout=10.0,
            )
...
class TestFailureScenarios:
    ...
    async def test_malformed_request(self):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{BASE_URL}/api/v1/chat", json={"invalid_field": "test"}, timeout=30.0
            )
